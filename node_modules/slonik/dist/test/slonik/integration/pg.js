...
test('returns expected query result object (NOTICE)', async (t) => {
    ...
    $$;
  `);
    const result = await pool.query(src_1.sql.unsafe `SELECT * FROM test_notice(${10});`);
...
test('streams rows', async (t) => {
  ...
  `);
    const messages = [];
    await pool.stream(src_1.sql.unsafe `
    SELECT name
    FROM person
  `, (stream) => {
...
test('streams over a transaction', async (t) => {
  ...
  `);
    ...
    await pool.transaction(async (transaction) => {
        await transaction.stream(src_1.sql.unsafe `
      SELECT name
      FROM person
    `, (stream) => {
            stream.on('data', (datum) => {
...
test('copies from binary stream', async (t) => {
    const pool = await (0, src_1.createPool)(t.context.dsn);
    await pool.copyFromBinary(src_1.sql.unsafe `
      COPY person
      (
        name
      )
    ...
    t.deepEqual(await pool.anyFirst(src_1.sql.unsafe `
    SELECT name
    FROM person
  `), [
        'foo',
...
test('throws error on syntax errors', async (t) => {
    const pool = await (0, src_1.createPool)(t.context.dsn);
    const error = await t.throwsAsync(pool.stream(src_1.sql.unsafe `NONSENSE NOT REALLY sql.fragment`, () => {
        t.false('We should not have got this far!');
    }));
    t.true(error instanceof Error);
    t.deepEqual(error === null || error === void 0 ? void 0 : error.message, 'syntax error at or near "NONSENSE"');
    await pool.end();
});
